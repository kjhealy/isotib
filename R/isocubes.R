...
isocubesGrob <- function(..., coords, fill = 'grey90', fill2 = NULL, fill3 = NULL,
  ...
  if (is.null(fill2)) {
    fill2 <- cheap_darken(fill, darkenby)
  } else {
    if (length(fill2) == 1) {
      fill2 <- rep(fill2, N)
  ...
  if (is.null(fill3)) {
    fill3 <- cheap_darken(fill, darkenby * 2)
  } else {
    if (length(fill3) == 1) {
...
isocubesGrobOrig <- function(coords, fill = NULL, fill2 = NULL, fill3 = NULL,
         light = 'top-left', darkenby = 0.3,
         ysize = 1/20, xo = 0.5, yo = ysize,
         verbose = FALSE, ...) {
  ...
  if (is.null(fill2)) {
    fill2 <- cheap_darken(fill, darkenby * 2)
  } else {
    if (length(fill2) == 1) {
      fill2 <- rep(fill2, N)
    } else if (length(fill2) != Norig) {
